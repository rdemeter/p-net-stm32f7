   ...
   while (port != 0)
   {
      ...
      else
      {
         /* In single port configuration the managed port is also
            physical port 1 */
    	 printf("           In single port configuration the managed port is also physical port\n");
...
int pf_eth_recv (pnal_eth_handle_t * eth_handle, void * arg, pnal_buf_t * p_buf)
{
   ...
   printf("      pf_eth_recv eth_type=0x%X\n", eth_type);
   switch (eth_type)
   {
